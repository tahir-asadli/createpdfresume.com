<div class="pt-5 pb-10 px-10">
    <div class="relative">
        <h2 class="font-semibold text-lg md:text-2xl pr-10">{{ __('Edit skill') }}</h2>
    ...
    <form class="flex flex-col gap-4 mt-4" wire:submit="submit">
        <div class="relative has-limit" limit="500">
            <label for="skill" class="required">{{ __('Skill') }}</label>
            <input type="text" placeholder="{{ __('Skill') }}" autocomplete="off" autofocus wire:model="form.name">
            <div class="absolute w-full flex justify-end text-gray-500 limit-indicator"><span><span
                        class="chars">{{ mb_strlen($this->form->name) }}</span>/500</span></div>
            @error('form.name')
        ...
        <div>
            ...
            <select wire:model="form.level">
                @foreach (range(5, 100, 5) as $item)
                    <option {{ $skill->level == $item ? 'selected' : '' }} value="{{ $item }}">
                        {{ $item }}</option>
                @endforeach
            </select>
            @error('form.level')
        ...
        <div>
            ...
            <x-switch id="active" value="{{ $skill->active }}" wire:model.live="form.active" />
        </div>
