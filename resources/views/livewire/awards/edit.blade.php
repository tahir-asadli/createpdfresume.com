<div class="pt-5 pb-10 px-10">
    <div class="relative">
        <h2 class="font-semibold text-lg md:text-2xl pr-10">{{ __('Edit award') }}</h2>
    ...
    <form class="flex flex-col gap-4 mt-4" wire:submit="submit" x-init="new AirDatepicker('#date', {
        locale: localeGlobal,
        position: 'top right',
        selectedDates: {{ $award->date ? '["' . $award->date->format('d-m-Y') . '"]' : '[]' }},
        onSelect({ formattedDate }) {
            $wire.form.date = formattedDate ? formattedDate : null;
        }
    });">
        <div class="relative has-limit" limit="100">
            <label class="required cursor-pointer" for="award">{{ __('Award name') }}</label>
            <input id="award" type="text" placeholder="{{ __('Award name') }}" autocomplete="off" autofocus wire:model="form.name">
        ...
        <div class="relative has-limit" limit="500">
            ...
            <div class="absolute w-full flex justify-end text-gray-500 limit-indicator"><span><span
                        class="chars">{{ mb_strlen($this->form->organization) }}</span>/500</span></div>
