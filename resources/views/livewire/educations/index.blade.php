<table class="w-full">
    <thead>
        <tr>
            <th class="text-lg text-left bg-slate-100 p-3 rounded-tl-md">{{ __('Place of education') }}</th>
            <th class="text-lg text-left bg-slate-100 p-3 hidden md:table-cell">{{ __('Degree') }}</th>
            <th class="text-lg text-left bg-slate-100 p-3 hidden md:table-cell">{{ __('Field') }}</th>
    ...
    <tbody wire:sortable="updateOrder">
        @foreach (auth()->user()->educations()->orderBy('order')->get() as $education)
            <tr wire:sortable.item="{{ $education->id }}" wire:key="education-{{ $education->id }}">
                <td class="p-2 border-b border-gray-100 font-bold">
                    <span wire:sortable.handle class="cursor-pointer"><x-icons.move /></span><span>{{ $education->school }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $education->degree }}</td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $education->field }}</td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">
                    {{ $education->start_date ? $education->start_date->format('d/m/Y') : '' }}
                    {{ $education->end_date ? ' - ' . $education->end_date->format('d/m/Y') : '' }}</td>
                ...
                <td class="p-2 border-b border-gray-100 text-center"><button
                        ...
                        onclick="Livewire.dispatch('openModal', { component: 'edit-education',arguments: {education: '{{ $education->id }}'} })"><x-icons.edit
                            class="md:hidden" /><span class="hidden md:inline">{{ __('Edit') }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 text-right">
                    <button wire:click="delete('{{ $education->id }}')" wire:confirm="{{ __('Are you sure?') }}"
