<div class="p-10 overflow-auto h-[100vh]">
    <div class="relative">
        <h2 class="font-semibold text-lg md:text-2xl pr-10">{{ __('Edit place of education') }}</h2>
    ...
    <form class="flex flex-col gap-4 mt-4" wire:submit="submit" x-init="new AirDatepicker('#education-startDate', {
        locale: localeGlobal,
        position: 'top right',
        selectedDates: {{ $education->start_date ? '["' . $education->start_date->format('d-m-Y') . '"]' : '[]' }},
        onSelect({ formattedDate }) {
            $wire.form.startDate = formattedDate ? formattedDate : null;
        }
    });
    new AirDatepicker('#education-endDate', {
        locale: localeGlobal,
        position: 'top right',
        selectedDates: {{ $education->end_date ? '["' . $education->end_date->format('d-m-Y') . '"]' : '[]' }},
        onSelect({ formattedDate }) {
        ...
        <div class="relative has-limit" limit="120">
            ...
            <input type="text" placeholder="{{ __('Place of education') }}" autocomplete="off" id="school" autofocus wire:model="form.school">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="cursor-pointer" for="education-startDate">{{ __('Start date') }}</label>
                <input placeholder="01-01-{{ date('Y') }}" type="text" id="education-startDate"
            ...
            <div>
                <label class="cursor-pointer" for="education-endDate">{{ __('End date') }}</label>
                <input placeholder="01-01-{{ date('Y') }}"type="text" id="education-endDate"
