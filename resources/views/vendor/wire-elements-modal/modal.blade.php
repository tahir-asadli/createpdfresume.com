<div>
    ...
    <div x-data="LivewireUIModal()" x-on:close.stop="setShowPropertyTo(false)"
        ...
        style="display: none;">
        {{-- flex items-end justify-center min-h-screen px-4 pt-4 pb-10 text-center sm:block sm:p-0 --}}
        {{--  min-h-screen --}}
        <div class="flex items-end justify-center h-full">
            ...
            <div x-show="show && showActiveComponent" x-transition:enter="ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-x-[100vw] sm:translate-x-[100vw]"
                x-transition:enter-end="opacity-100 translate-x-0 " x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave-end="opacity-0 translate-y-4 sm:translate-x-[100vw]" x-bind:class="modalWidth"
                class="inline-block w-full align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-full"
                {{-- x-trap.noscroll.inert="show && showActiveComponent" --}} id="modal-container" aria-modal="true">
