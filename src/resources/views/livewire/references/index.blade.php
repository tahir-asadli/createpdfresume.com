<table class="w-full">
    <thead>
        <tr>
            <th class="text-lg text-left bg-slate-100 p-3 rounded-tl-md">{{ __('Fullname') }}</th>
            <th class="text-lg text-left bg-slate-100 p-3 hidden md:table-cell">{{ __('Company') }}</th>
    ...
    <tbody wire:sortable="updateOrder">
        @foreach (auth()->user()->references()->orderBy('order')->get() as $reference)
            <tr  wire:sortable.item="{{ $reference->id }}" wire:key="reference-{{ $reference->id }}">
                <td class="p-2 border-b border-gray-100 font-bold"><span wire:sortable.handle class="cursor-pointer"><x-icons.move /></span><span>{{ $reference->name }}</span></td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $reference->company }}</td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $reference->position }}</td>
                ...
                <td class="p-2 border-b border-gray-100 text-center"><button
                        ...
                        onclick="Livewire.dispatch('openModal', { component: 'edit-reference',arguments: {reference: '{{ $reference->id }}'} })"><x-icons.edit
                            class="md:hidden" /><span class="hidden md:inline">{{ __('Edit') }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 text-right">
                    <button wire:click="delete('{{ $reference->id }}')" wire:confirm="{{ __('Are you sure?') }}"
