<table class="w-full">
    <thead>
        <tr>
            <th class="text-lg text-left bg-slate-100 p-3 rounded-tl-md">{{ __('Project name') }}</th>
    ...
    <tbody wire:sortable="updateOrder">
        @foreach (auth()->user()->projects()->orderBy('order')->get() as $project)
            <tr  wire:sortable.item="{{ $project->id }}" wire:key="project-{{ $project->id }}">
                <td class="p-2 border-b border-gray-100 font-bold">
                    <span wire:sortable.handle class="cursor-pointer"><x-icons.move /></span><span>{{ $project->name }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">
                    {{ $project->date ? $project->date->format('d/m/Y') : '' }}
                </td>
                <td class="p-2 border-b border-gray-100 font-bold hidden md:table-cell">{{ $project->url }}</td>
                ...
                <td class="p-2 border-b border-gray-100 text-center"><button
                        ...
                        onclick="Livewire.dispatch('openModal', { component: 'edit-project',arguments: {project: '{{ $project->id }}'} })"><x-icons.edit
