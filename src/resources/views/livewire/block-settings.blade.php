        ...
        <form class="flex flex-col gap-4 mt-4" wire:submit="submit">
            ...
            @if (in_array($block->widget->name, ['ul', 'ol']))
                <div>
                    ...
                    <textarea placeholder="{{ __('Content') }}" rows="10" autofocus wire:model="form.title"></textarea>
                    @error('form.title')
                        <p class="text-rose-600 italic pt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            @else
                ...
                @if ($block->widget->name == 'image')
                    ...
                    @if ($this->hasTransparentImage())
                        <div class="flex gap-2 flex-wrap">
                            <div class="button-group">
                                <button type="button" wire:click="toggleTransparent()"
                    ...
                    <div class="flex gap-2 flex-wrap flex-col">
                        @foreach ($this->blendModes() as $blendModeGroup)
                            <div class="button-group">
                                @foreach ($blendModeGroup as $blendMode)
                                    <button type="button" wire:click="setBlendMode('{{ $blendMode }}')"
            ...
            @if (in_array($block->widget->name, [
                ...
                <div class="flex gap-2 flex-wrap">
                    <div class="button-group">
                        <button type="button" wire:click="setAlignment('left')"
                            :class="{
                                '!shadow-cs-inset': '{{ $form->align == 'left' }}'
                            }"
                            class="button"><x-icons.align_left /></button>
                        <button type="button" wire:click="setAlignment('center')"
                            :class="{
                                '!shadow-cs-inset': '{{ $form->align == 'center' }}'
                            }"
                            class="button"><x-icons.align_center /></button>
                        <button type="button" wire:click="setAlignment('right')"
                            :class="{
