<div class="p-10 overflow-auto h-[100vh]">
    ...
    <div class="relative">
        <h2 class="font-semibold text-lg md:text-2xl pr-10">{{ __('Edit workplace') }}</h2>
    ...
    <form class="flex flex-col gap-4 mt-4" wire:submit="submit" x-init="new AirDatepicker('#startDate', {
        locale: localeGlobal,
        position: 'top right',
        selectedDates: {{ $experience->start_date ? '["' . $experience->start_date->format('d-m-Y') . '"]' : '[]' }},
    ...
    new AirDatepicker('#endDate', {
        locale: localeGlobal,
        position: 'top right',
        selectedDates: {{ $experience->end_date ? '["' . $experience->end_date->format('d-m-Y') . '"]' : '[]' }},
        onSelect({ formattedDate }) {
