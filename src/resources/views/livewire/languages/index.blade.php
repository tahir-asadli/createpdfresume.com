<table class="w-full">
    <thead>
        <tr>
            <th class="text-lg text-left bg-slate-100 p-3 rounded-tl-md">{{ __('Language') }}</th>
            <th class="text-lg text-center w-40 bg-slate-100 p-3 hidden md:table-cell">{{ __('Percentage') }}</th>
    ...
    <tbody wire:sortable="updateOrder">
        @foreach (auth()->user()->languages()->orderBy('order')->get() as $language)
            <tr wire:sortable.item="{{ $language->id }}" wire:key="language-{{ $language->id }}">
                <td class="p-2 border-b border-gray-100 font-bold">
                    <span wire:sortable.handle class="cursor-pointer"><x-icons.move /></span><span>{{ $language->name }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell text-center">{{ $language->level }}%</td>
                ...
                <td class="p-2 border-b border-gray-100 text-center"><button
                        ...
                        onclick="Livewire.dispatch('openModal', { component: 'edit-language',arguments: {language: '{{ $language->id }}'} })"><x-icons.edit
                            class="md:hidden" /><span class="hidden md:inline">{{ __('Edit') }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 text-right">
                    <button wire:click="delete('{{ $language->id }}')" wire:confirm="{{ __('Are you sure?') }}"
