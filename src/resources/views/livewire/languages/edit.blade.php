<div class="pt-5 pb-10 px-10">
    <div class="relative">
        <h2 class="font-semibold text-lg md:text-2xl pr-10">{{ __('Edit language') }}</h2>
    ...
    <form class="flex flex-col gap-4 mt-4" wire:submit="submit">
        ...
        <div>
            ...
            <select wire:model="form.level">
                @foreach (range(5, 100, 5) as $item)
                    <option {{ $language->level == $item ? 'selected' : '' }} value="{{ $item }}">
                        {{ $item }}</option>
                @endforeach
            </select>
            @error('form.level')
        ...
        <div>
            ...
            <x-switch id="active" value="{{ $language->active }}" wire:model.live="form.active" />
        </div>
