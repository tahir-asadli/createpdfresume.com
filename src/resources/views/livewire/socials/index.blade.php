<table class="w-full">
    <thead>
        <tr>
            <th class="text-lg text-left bg-slate-100 p-3 rounded-tl-md">{{ __('Site') }}</th>
            <th class="text-lg text-left bg-slate-100 p-3 hidden md:table-cell">{{ __('URL') }}</th>
            <th class="text-lg text-left bg-slate-100 p-3 hidden md:table-cell">{{ __('Username') }}</th>
    ...
    <tbody wire:sortable="updateOrder">
        @foreach (auth()->user()->socials()->orderBy('order')->get() as $social)
            <tr  wire:sortable.item="{{ $social->id }}" wire:key="social-{{ $social->id }}">
                <td class="p-2 border-b border-gray-100 font-bold">
                    <span class="flex gap-4">
                        ...
                        <span class="small-social"><x-dynamic-component
                                component="socials.{{ $social->site }}" /></span>
                        <span>
                            {{ ucfirst($social->site) }}{{ $social->name ? ' - (' . $social->name . ')' : '' }}
                </td>
                </span>
                </span>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $social->url }}</td>
                <td class="p-2 border-b border-gray-100 hidden md:table-cell">{{ $social->handle }}</td>
                ...
                <td class="p-2 border-b border-gray-100 text-center"><button
                        ...
                        onclick="Livewire.dispatch('openModal', { component: 'edit-social',arguments: {social: '{{ $social->id }}'} })"><x-icons.edit
                            class="md:hidden" /><span class="hidden md:inline">{{ __('Edit') }}</span>
                </td>
                <td class="p-2 border-b border-gray-100 text-right">
                    <button wire:click="delete('{{ $social->id }}')" wire:confirm="{{ __('Are you sure?') }}"
